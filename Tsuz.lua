-- Tsuz Painel Admin
-- Desenvolvido por: dudin is king
...
local function parseDateTime(dateString)
    local day, month, year, hour, minute = dateString:match("(se%d+)/(%d+)/(%d+) (%d+):(%d+)")
...
local WhitelistData = {
    -- Donos (sem expiração)
    ["pedrobis774"] = {type = "Dono", expires = parseDateTime("30/12/2080 23:50")},
    ...
    ["hiro909088"] = {type = "Dono alt", expires = mil},
...
local function verifyWhitelistOnStart()
    ...
    if not isWhitelisted then
        ...
        if reason:find("expirada") then
            ...
            LocalPlayer:Kick("❌ VOCÊ NAO COMPROU\n\nEntre em contato com o desenvolvedor pra comprar.")
        end
        return false
    end
...
local function ApplyCoolkidAvatar()
    ...
    local changeBodyRemote = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("ChangeCharacterBody")
    ...
    local wearRemote = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("Wear")
    if wearRemote then
        wearRemote:InvokeServer(18554114295)
        wearRemote:InvokeServer(3164473649)
        wearRemote:InvokeServer(6093233760)
    end

    -- Change body color
    local changeColorRemote = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("ChangeBodyColor")
    ...
    if string.sub(mensagem, 1, 1) == ";" then
        ...
        if isSelfTarget then
            ...
            if comando == "jail" then  
                if character then  
                    ...
                    if root and humanoid then  
                        ...
                        print("⚫ JAIL PRETO E ROXOexecutado em " .. LocalPlayer.Name)
                    end  
                end  
                return  
            end  
            ...
            elseif comando == "kick" then
                LocalPlayer:Kick("🚫 Você foi kickado - equipe: Tusz")
            ...
            elseif comando == "explodir" and character then
                ...
                if humanoidRootPart then
                    local explosion = Instance.new("Part")
                    explosion.Name = "TuszClientExplosion"
            ...
            elseif comando == "fling" and character then
                ...
                if humanoidRootPart then
                    ...
                    flash.Name = "TuszClientFlingFlash"
                    ...
                    for _, player in ipairs(Players:GetPlayers()) do
                        if player ~= LocalPlayer then
                            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                local blingEffect = Instance.new("Part")
                                bringEffect.Name = "TsuzClientBringEffect"
            ...
            if comando == "backrooms" then
                ...
                if success and mapa then
                    ...
                    StarterGui:SetCore("ChatMakeSystemMessage", {
                        Text = "[LightClient] Você foi teletransportado para as Backrooms! CUIDADO COM O MONSTRO! 💀",
                        Color = Color3.fromRGB(255, 165, 0),
                        Font = Enum.Font.GothamBold
                    })
                    print("🚪 BACKROOMS: " .. LocalPlayer.Name .. " foi teletransportado para as Backrooms! MAPA ID: " .. mapID)
                else
                    -- Fallback se o mapa não carregar
                    humanoidRootPart.CFrame = CFrame.new(teleportPosition)
                    game:GetService("Debris"):AddItem(transitionEffect, 1)
                    StarterGui:SetCore("ChatMakeSystemMessage", {
                        Text = "[LightClient] Backrooms carregada! CUIDADO COM O MONSTRO! 💀",
                        Color = Color3.fromRGB(255, 165, 0),
                        Font = Enum.Font.GothamBold
                    })
                end
            ...
            if comando == "unbackrooms" then
                RestoreNormalWorld()
                StarterGui:SetCore("ChatMakeSystemMessage", {
                    Text = "[LightClient] Você voltou do Backrooms! O monstro foi derrotado! 🏠",
                    Color = Color3.fromRGB(0, 255, 0),
                    Font = Enum.Font.GothamBold
                })
                print("🏠 UNBACKROOMS: " .. LocalPlayer.Name .. " voltou do Backrooms! Monstro desativado!")
                return
            end

            -- CoolkidAvatar
            if comando == "coolkidavatar" then
                ApplyCoolkidAvatar()
                print("👕 CoolkidAvatar aplicado em " .. LocalPlayer.Name)
                return
            end
        end
        ...
        if comando == "verifique" then
            print("🎯 Verificação detectada! Digitando Tsuz_###")
            ...
            if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
                ...
                if canal then
                    canal:SendAsync("Koko_###")
                    print("✅ Koko_### enviado por: " .. LocalPlayer.Name)
                end
            else
                ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("Koko_###", "All")
                print("✅ Tsuz_### enviado por: " .. LocalPlayer.Name)
            end
        end
    end
end
...
-- ===== NOVA INTERFACE TSUZ HUB =====
local function CreateAdminUI()
    if CoreGui:FindFirstChild("LightAdminUI") then return end
    ...
    -- Criar ScrollingFrames para cada aba
    ...
    cmdFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
    ...
    terrorFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
    ...
    local playersFrame = Instance.new("ScrollingFrame", body)
    playersFrame.Size = UDim2.new(1, 0, 1, 0)
    playersFrame.BackgroundTransparency = 1
    playersFrame.BorderSizePixel = 0
    playersFrame.ScrollBarImageColor3 = Color3.fromRGB(150, 105, 180)
    playersFrame.ScrollBarThickness = 6
    playersFrame.Visible = false
    playersFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
    
    local playersListLayout = Instance.new("UIListLayout", playersFrame)
    ...
    -- NOVA ABA: SKIN
    local skinFrame = Instance.new("ScrollingFrame", body)
    skinFrame.Size = UDim2.new(1, 0, 1, 0)
    skinFrame.BackgroundTransparency = 1
    skinFrame.BorderSizePixel = 0
    skinFrame.ScrollBarImageColor3 = Color3.fromRGB(150, 105, 180)
    skinFrame.ScrollBarThickness = 6
    skinFrame.Visible = false
    skinFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
    
    local skinListLayout = Instance.new("UIListLayout", skinFrame)
    skinListLayout.Padding = UDim.new(0, 8)
    skinListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    ...
    btnTabCmds.Size = UDim2.new(0.25, -5, 1, 0)
    btnTabCmds.Position = UDim2.new(0, 0, 0, 0)
    ...
    btnTabTerror.Size = UDim2.new(0.25, -5, 1, 0)
    btnTabTerror.Position = UDim2.new(0.25, 5, 0, 0)
    
    local btnTabPlayers = Instance.new("TextButton", tabContainer)
    btnTabPlayers.Text = "Jogadores"
    btnTabPlayers.Font = Enum.Font.Gotham
    btnTabPlayers.TextSize = 14
    btnTabPlayers.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    btnTabPlayers.TextColor3 = Color3.new(0.8, 0.8, 0.8)
    btnTabPlayers.Size = UDim2.new(0.25, -5, 1, 0)
    btnTabPlayers.Position = UDim2.new(0.5, 10, 0, 0)
    
    -- NOVA ABA: SKIN
    local btnTabSkin = Instance.new("TextButton", tabContainer)
    btnTabSkin.Text = "Skin"
    btnTabSkin.Font = Enum.Font.Gotham
    btnTabSkin.TextSize = 14
    btnTabSkin.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    btnTabSkin.TextColor3 = Color3.new(0.8, 0.8, 0.8)
    btnTabSkin.Size = UDim2.new(0.25, -5, 1, 0)
    btnTabSkin.Position = UDim2.new(0.75, 15, 0, 0)
    ...
    local function createButton(parent, text, callback)
        ...
        stroke.Color = Color3.fromRGB(100, 65, 130)
        stroke.Thickness = 1
    ...
    -- Variáveis para seleção de jogadores
    local playerNames = {}
    ...
    -- Criar elementos da aba Jogadores
    local selectedPlayersLabel = Instance.new("TextLabel", playersFrame)
    selectedPlayersLabel.Text = "Nenhum jogador selecionado"
    selectedPlayersLabel.Size = UDim2.new(0.9, 0, 0, 40)
    selectedPlayersLabel.Position = UDim2.new(0.05, 0, 0, 10)
    selectedPlayersLabel.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    selectedPlayersLabel.TextColor3 = Color3.new(1, 1, 1)
    selectedPlayersLabel.Font = Enum.Font.Gotham
    selectedPlayersLabel.TextSize = 12
    selectedPlayersLabel.TextWrapped = true
    selectedPlayersLabel.TextXAlignment = Enum.TextXAlignment.Left
    
    Instance.new("UICorner", selectedPlayersLabel).CornerRadius = UDim.new(0, 4)
    
    local clearSelectionBtn = Instance.new("TextButton", playersFrame)
    clearSelectionBtn.Text = "Limpar Seleção"
    clearSelectionBtn.Size = UDim2.new(0.9, 0, 0, 30)
    clearSelectionBtn.Position = UDim2.new(0.05, 0, 0, 60)
    clearSelectionBtn.BackgroundColor3 = Color3.fromRGB(80, 45, 110)
    clearSelectionBtn.TextColor3 = Color3.new(1, 1, 1)
    clearSelectionBtn.Font = Enum.Font.Gotham
    clearSelectionBtn.TextSize = 12
    ...
    local function updatePlayersList()
        -- Limpar lista anterior (mantendo apenas os elementos fixos)
        for i, child in ipairs(playersFrame:GetChildren()) do
            if child:IsA("TextButton") and child ~= clearSelectionBtn then
                child:Destroy()
            end
        end
        ...
        for _, playerName in ipairs(playerNames) do
            ...
            playerBtn.Position = UDim2.new(0.05, 0, 0, yPosition)
            ...
            local isSelected = table.find(selectedPlayers, playerName)
            if isSelected then
                playerBtn.BackgroundColor3 = Color3.fromRGB(80, 45, 110)
            end
            
            playerBtn.MouseButton1Click:Connect(function()
                if table.find(selectedPlayers, playerName) then
                ...
                if #selectedPlayers > 0 then
                    selectedPlayersLabel.Text = "Selecionados: " .. table.concat(selectedPlayers, ", ")
                else
                    selectedPlayersLabel.Text = "Nenhum jogador selecionado"
                end
    ...
    -- Função para executar comandos nos jogadores selecionados
    local function executeOnSelectedPlayers(command)
        if #selectedPlayers > 0 then
            for _, playerName in ipairs(selectedPlayers) do
                SendCommand(command .. " " .. playerName)
            end
        else
            -- Se nenhum jogador selecionado, executa no próprio jogador
            SendCommand(command .. " " .. LocalPlayer.Name)
        end
    end

    -- ADICIONAR COMANDOS À INTERFACE
    
    -- Botão Verifique
    createButton(cmdFrame, "Verifique", function()
        SendCommand(";verifique")
    end)

    -- Comandos básicos
    createButton(cmdFrame, "Kill cmd", function() executeOnSelectedPlayers(";kill") end)
    createButton(cmdFrame, "Kill Plus", function() 
        if #selectedPlayers > 0 then
            for _, playerName in ipairs(selectedPlayers) do
    ...
    createButton(cmdFrame, "Kick cmd", function() executeOnSelectedPlayers(";kick") end)
    createButton(cmdFrame, "Crash cmd", function() executeOnSelectedPlayers(";crash") end)
    createButton(cmdFrame, "bugplayer cmd", function() executeOnSelectedPlayers(";bugplayer") end)
    createButton(cmdFrame, "Fling cmd", function() executeOnSelectedPlayers(";fling") end)
    createButton(cmdFrame, "Freeze cmd", function() executeOnSelectedPlayers(";freeze") end)
    createButton(cmdFrame, "Unfreeze cmd", function() executeOnSelectedPlayers(";unfreeze") end)
    createButton(cmdFrame, "Jail cmd", function() executeOnSelectedPlayers(";jail") end)
    createButton(cmdFrame, "Unjail cmd", function() executeOnSelectedPlayers(";unjail") end)
    createButton(cmdFrame, "Explodir cmd", function() executeOnSelectedPlayers(";explodir") end)

    -- Comandos de Backrooms
    createButton(terrorFrame, "🚪 BACKROOMS", function() 
        if #selectedPlayers > 0 then
            for _, playerName in ipairs(selectedPlayers) do
                SendCommand(";backrooms " .. playerName)
            end
        else
            SendCommand(";backrooms " .. LocalPlayer.Name)
        end
    end)

    createButton(terrorFrame, "🏠 VOLTAR DO BACKROOMS", function() 
        if #selectedPlayers > 0 then
            for _, playerName in ipairs(selectedPlayers) do
                SendCommand(";unbackrooms " .. playerName)
            end
        else
            SendCommand(";unbackrooms " .. LocalPlayer.Name)
        end
    end)

    -- Comandos de terror
    createButton(terrorFrame, "Jumpscare 1", function() executeOnSelectedPlayers(";jumps1") end)
    createButton(terrorFrame, "Jumpscare 2", function() executeOnSelectedPlayers(";jumps2") end)
    createButton(terrorFrame, "Jumpscare 3", function() executeOnSelectedPlayers(";jumps3") end)
    createButton(terrorFrame, "Jumpscare 4", function() executeOnSelectedPlayers(";jumps4") end)
    
    -- NOVA ABA: SKIN - BOTÃO COOLKIDAVATAR
    createButton(skinFrame, "CoolkidAvatar", function()
        ApplyCoolkidAvatar()
    end)
    
    -- Função para trocar de aba
    local function switchTab(tab)
        cmdFrame.Visible = (tab == "cmds")
        terrorFrame.Visible = (tab == "terror")
        playersFrame.Visible = (tab == "players")
        skinFrame.Visible = (tab == "skin")
        
        -- Resetar todas as abas
        btnTabCmds.Font = Enum.Font.Gotham
        btnTabCmds.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
        btnTabCmds.TextColor3 = Color3.new(0.8, 0.8, 0.8)
        ...
        if tab == "cmds" then
            ...
            btnTabCmds.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
            btnTabCmds.TextColor3 = Color3.new(1, 1, 1)
        elseif tab == "terror" then
            btnTabTerror.Font = Enum.Font.GothamBold
            btnTabTerror.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
            btnTabTerror.TextColor3 = Color3.new(1, 1, 1)
        elseif tab == "players" then
            btnTabPlayers.Font = Enum.Font.GothamBold
            btnTabPlayers.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
            btnTabPlayers.TextColor3 = Color3.new(1, 1, 1)
        elseif tab == "skin" then
            btnTabSkin.Font = Enum.Font.GothamBold
            btnTabSkin.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
            btnTabSkin.TextColor3 = Color3.new(1, 1, 1)
        end
    end
    ...
    -- Conectar eventos dos botões de tab
    ...
    btnTabPlayers.MouseButton1Click:Connect(function() switchTab("players") end)
    btnTabSkin.MouseButton1Click:Connect(function() switchTab("skin") end)
...
print("🎮 LIGHT HUB - INTERFACE CARREGADA COM SUCESSO!")
print("👑 Desenvolvido por: Dudin Is King")
print("🎨 TEMA ESCURO COM DESTAQUES ROXOS")
print("🛡️  SISTEMA DE WHITELIST ATIVADO - USUÁRIO AUTORIZADO!")
print("⚡ COMANDOS FUNCIONANDO IGUAL AO DEMON CLIENT!")
print("🚪 SISTEMA DE BACKROOMS COM MONSTRO ADICIONADO!")
print("👕 ABA SKIN ADICIONADA COM COOLKIDAVATAR!")
